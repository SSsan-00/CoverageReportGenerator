...
出力先には `CoverageReportGenerator.exe` と `source/` が作成されます。`source/` にはアプリ本体とテストコードを含みます。

bootstrap の成果物には `bootstrap/`、`docs/`、`README.md` は含まれません。bootstrap に関する説明は、このリポジトリ側のドキュメントだけに残します。
